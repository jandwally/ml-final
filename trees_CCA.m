function pred_labels=predict_labels4(train_inputs,train_labels,test_inputs)
...
for f = 1:d
    ...
    x = X_train;
    y = y_train;
    z = inv(sqrt(x' * x)) * x' * y * inv(sqrt(y' * y));
    % Perform CCA: find SVD of Z
    [U,S,V] = svd(z); 
    X_train = X_train * U(:,1:k);
    y_train = y_train * V';
    X_test = X_test * U(:,1:k);
...
%}
    ...
    % Predicting...
    pred_labels(:,f) = predict(tree,X_test) * inv(V');
end
